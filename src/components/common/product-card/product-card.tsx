...
const ProductCard: FunctionComponent<Props> = ({image, name, desc, reference, stock, price, discount, type}) => {
  ...
  return (
    <div className={'product-card' + (dark ? ' dark-background' : '')}>
        ...
        <a href="#" className={'product-card-name' + (dark ? ' dark-color' : '')}>{name}</a>
        <p className='product-card-desc'>{desc}</p>
        <p className={'product-card-ref' + (dark ? ' dark-color' : '')}>{reference}</p>
        ...
        <div className='product-card-price'>
            ...
            {promotion && <div className='product-card-pvpr'>
                <p>PVPR<br/>
                <span className={dark ? 'dark-color' : ''}>{price.toFixed(2).replace('.', ',')}€</span>
                </p>
            </div>}
        </div>
