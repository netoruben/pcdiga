...
const Carousel: FunctionComponent<Props> = ({title, type}) => {
    ...
    const carouselHideLeft = useRef<HTMLDivElement>(null)
    ...
    const remainingSeconds = Math.floor((utc2 - utc1) / 1000);

    const horas = useCountdown({ isPlaying: true, duration: remainingSeconds, colors: '#FD6205' })
    const minutos = useCountdown({ isPlaying: true, duration: remainingSeconds / Math.ceil(remainingSeconds / (60 * 60)), colors: '#FD6205', onComplete: () => {
        if (horas.remainingTime > 0 && horas.remainingTime <= 3600)
            return { shouldRepeat: true, newInitialRemainingTime: horas.remainingTime}
        return { shouldRepeat: true }
    }})
    const segundos = useCountdown({ isPlaying: true, duration: (remainingSeconds / Math.ceil(remainingSeconds / (60 * 60))) / 60, colors: '#FD6205', onComplete: () => {
        if (minutos.remainingTime > 0 && minutos.remainingTime <= 60)
            return { shouldRepeat: true, newInitialRemainingTime: minutos.remainingTime}
        return { shouldRepeat: true }
    }})
    ...
    window.addEventListener(('resize'), () => {
        if (draggable.current && carouselHideLeft.current && carouselHideRight.current) {
            carouselHideLeft.current.style.left = '-' + draggable.current.clientWidth.toString() + 'px'
            carouselHideRight.current.style.left = draggable.current.clientWidth.toString() + 'px'
        }
    })
    ...
    if (type !== 'blog') {
        ...
        NewsData.forEach((single_news) => {
            products.push(<NewsCard image={single_news.image} title={single_news.title} subtitle={single_news.subtitle}/>)
          })
    }
  ...
  return (
    <section className='carousel'>
        ...
        <div className='carousel-products'>
            <div ref={draggable} onMouseDown={(ev) => mouseDown(ev)} onMouseMove={(ev) => mouseMove(ev)} onMouseUp={(ev) => mouseUp(ev)} onTouchEnd={(ev) => mouseUp(ev)} onTouchMove={(ev) => mouseMove(ev)} onTouchStart={(ev) => mouseDown(ev)} className='carousel-draggable'/>
            <div ref={carousel} className='carousel-movable'>
                    ...
                    <div className='carousel-countdown-circles'>
                        <div className='carousel-countdown-circle'>
                            <div className='carousel-countdown-circle-inner'>
                                <p>{Math.ceil(horas.remainingTime / (60 * 60))}</p>
                            </div>
                            <svg viewBox="-10 0 200 180" width="70" height="70" xmlns="http://www.w3.org/2000/svg">
                                <path d="m 90,6 a 84,84 0 1,0 0,168 a 84,84 0 1,0 0,-168" fill="none" stroke="#001839" strokeWidth="16"/>
                                <path d={horas.path} fill="none" stroke={horas.stroke} strokeLinecap="round" strokeWidth="12" strokeDasharray="527.7875658030853" strokeDashoffset={horas.strokeDashoffset}/>
                            </svg>
                            <p>horas</p>
                        </div>
                        <div className='carousel-countdown-circle'>
                            <div className='carousel-countdown-circle-inner'>
                                <p>{Math.ceil(minutos.remainingTime / 60)}</p>
                            </div>
                            <svg viewBox="-10 0 200 180" width="70" height="70" xmlns="http://www.w3.org/2000/svg">
                                <path d="m 90,6 a 84,84 0 1,0 0,168 a 84,84 0 1,0 0,-168" fill="none" stroke="#001839" strokeWidth="16"/>
                                <path d={minutos.path} fill="none" stroke={minutos.stroke} strokeLinecap="round" strokeWidth="12" strokeDasharray="527.7875658030853" strokeDashoffset={minutos.strokeDashoffset}/>
                            </svg>
                            <p>minutos</p>
                        </div>
                        <div className='carousel-countdown-circle'>
                            <div className='carousel-countdown-circle-inner'>
                                <p>{segundos.remainingTime}</p>
                            </div>
                            <svg viewBox="-10 0 200 180" width="70" height="70" xmlns="http://www.w3.org/2000/svg">
                                <path d="m 90,6 a 84,84 0 1,0 0,168 a 84,84 0 1,0 0,-168" fill="none" stroke="#001839" strokeWidth="16"/>
                                <path d={segundos.path} fill="none" stroke={segundos.stroke} strokeLinecap="round" strokeWidth="12" strokeDasharray="527.7875658030853" strokeDashoffset={segundos.strokeDashoffset}/>
                            </svg>
                            <p>segundos</p>
                        </div>
                    </div>
